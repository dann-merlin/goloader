...
func PatchGC(goBinary string, debugLog bool) error {
	...
	if bytes.Index(objFile, []byte(objSnippet)) == -1 {
		...
		if err != nil {
			if strings.Contains(err.Error(), "permission denied") || strings.Contains(err.Error(), "not permitted") {
				return fmt.Errorf("could not write patched '%s': %w\nTry changing $GOROOT's owner to current user, or run patch with sudo\ngo install github.com/dann-merlin/goloader/jit/patchgc@latest && sudo $GOPATH/bin/patchgc", objPath, err)
			}
	...
	if err != nil {
		if strings.Contains(err.Error(), "permission denied") || strings.Contains(err.Error(), "not permitted") {
			return fmt.Errorf("could not write patched '%s': %w\nTry changing $GOROOT's owner to current user, or run patch with sudo\ngo install github.com/dann-merlin/goloader/jit/patchgc@latest && sudo $GOPATH/bin/patchgc", goCompilerPath+".bak", err)
		}
	...
	if err != nil {
		if strings.Contains(err.Error(), "permission denied") || strings.Contains(err.Error(), "not permitted") {
			return fmt.Errorf("could not write patched '%s': %w\nTry changing $GOROOT's owner to current user, or run patch with sudo\ngo install github.com/dann-merlin/goloader/jit/patchgc@latest && sudo $GOPATH/bin/patchgc", goCompilerPath, err)
		}
